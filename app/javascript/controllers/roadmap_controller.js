...
export default class extends Controller {
  ...
  roadmapTargetConnected(target) {
    ...
    .then(response => {
      const items = {
        "drawflow": {
        "Home": {
          "data": response.data.attributes.nodes
        }}}

        window.editor = new Drawflow(target);
  ...
  export() {
    fetch(`${window.location.href}/build_nodes`, {
      method: 'PATCH',
      body: JSON.stringify({nodes: editor.export().drawflow.Home.data}),
      headers: {
        'Accept': 'application/json',
        'Content-Type': 'application/json',
  ...
  createNode(event) {
    ...
    fetch(`${event.srcElement.action}.json`, {
      method: 'POST',
      body: JSON.stringify(formDataToJson(new FormData(event.srcElement))),
      headers: {
        'Accept': 'application/json',
        'Content-Type': 'application/json',
        'X-CSRF-Token': document.head.querySelector(`meta[name="csrf-token"]`).getAttribute('content')
    ...
    .then(response => response.json())
    .then(response => {
      this.modalOutlet.hideModal();
      const node = response.data.attributes.node
      editor.addNodeOutput(node.inputs.input_1.connections[0].node);
      editor.addNode(node.name, 1, 3, node.pos_x, node.pos_y, node.class, node.data, node.html)
      editor.addConnection(node.inputs.input_1.connections[0].node, node.id, 'output_1', 'input_1')
    })
  }
}
